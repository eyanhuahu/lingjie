...
　已到设定天数：史诗 Boss（`epic`）必定深度入魔；其他生物 轻度 50%、中度 25%、深度 25%。
...
攻击 45，手持增加 10% 移动速度；攻击范围不再单独覆盖，改用玩家默认攻击距离。
...
射程 10，耐久 200。攻击间隔不再单独覆盖，与长矛等普通武器一致（射击动画两倍速，放箭帧从第 23 帧提前到第 12 帧）。攻击一次减 1 点耐久。
...
耐久 300。攻击间隔与武器攻击距离都不再单独覆盖，与长矛等普通武器一致。攻击一次减 1 点耐久。
耐久为 0 不消失，可继续攻击，但伤害降为 10。可用魔核填充耐久，1 个魔核填充 100。
...
线索 Boss 的入魔等级同样按世界天数抽取（见「入魔总览」：到达设定天数后，史诗 Boss 必定深度入魔），不受季节限制。
