...
作用：把《灵界设定.docx》的文字内容整理成站点读取的 data.json。
...
def R(*parts):
    ...
    数量为 None 时只写名称（例如"冰霜业火本体"）。
    """
    chunks = []
    for part in parts:
        name, qty = part[0], part[1]
...
TELE = [
    ...
    ("lj_alchemy_furnace", "丹炉"),
    ("lj_reiki_gourd", "灵虚葫"),
    ("lj_soul_banner", "魂幡"),
    ("lj_magic_ring", "虚空戒"),
    ("zhenfa_overview", "阵法"),
    ("lj_warcraft_forest", "魔兽森林"),
...
def item(sec, iid, name, tags, summary, detail, recipe="", image=""):
    if iid in _ORDER:
...
# ===========================================================================
item(
    "jingjie", "realm_system", "境界体系", "境界,修炼,升阶",
    "九大境界，每境九阶。靠击杀生物积累经验升阶，死亡会掉一阶。",
    """
新增境界值与灵力值，类似三维显示。

境界共 9 个：凡境、淬体、炼筋、辟谷、引气、入微、超凡、合婴、具灵。每个境界分 9 阶，如淬体一阶、二阶……九阶。

升阶方式：击杀生物获得经验。在 15 个地皮范围内、30 秒内参与攻击即可获得经验。

死亡惩罚：死亡一次掉一阶。掉阶后必须服用清心丸才能继续修炼，否则会止步不前。

瓶颈：所有境界的第 1 到 3 阶无瓶颈；第 3 阶升第 4 阶需要打坐；第 6 阶升第 7 阶需要打坐；第 9 阶升入下一个境界需要打坐。

引气及以上境界，须服用特定丹药并打坐 10 秒才能提升境界。
""",
)

item(
    "jingjie", "realm_exp", "经验值表", "境界,经验,数值",
    "九大境界、每境九阶的完整升阶经验需求。",
    """
凡境：一阶 50，二阶 100，三阶 150，四阶 200，五阶 250，六阶 300，七阶 350，八阶 450，九阶 500

淬体：一阶 500，二阶 600，三阶 700，四阶 800，五阶 900，六阶 1000，七阶 1100，八阶 1200，九阶 1300（每阶加 100）

炼筋：一阶 1300，二阶 1500，三阶 1700，四阶 1900，五阶 2100，六阶 2300，七阶 2500，八阶 2700，九阶 2900（每阶加 200）

辟谷：一阶 2900，二阶 3300，三阶 3700，四阶 4100，五阶 4500，六阶 4900，七阶 5300，八阶 5700，九阶 6100（每阶加 400）

引气：一阶 6100，二阶 6900，三阶 7700，四阶 8500，五阶 9300，六阶 10100，七阶 10900，八阶 11700，九阶 12500（每阶加 800）

入微：一阶 12500，二阶 14000，三阶 15500，四阶 17000，五阶 18500，六阶 20000，七阶 21500，八阶 23000，九阶 24500（每阶加 1500）

超凡：一阶 24500，二阶 27000，三阶 29500，四阶 32000，五阶 34500，六阶 37000，七阶 39500，八阶 42000，九阶 44500（每阶加 2500）

合婴：一阶 44500，二阶 48500，三阶 52500，四阶 56500，五阶 60500，六阶 64500，七阶 68500，八阶 72500，九阶 76500（每阶加 4000）

具灵：一阶 76500，二阶 82500，三阶 88500，四阶 94500，五阶 100500，六阶 106500，七阶 112500，八阶 118500，九阶 124500（每阶加 6000）

经验获取量 = 生物血量 × 5.8.12%（可调节）。

瓶颈限制：每三阶需要打坐。例如当前引气三阶、经验 8000，升四阶只差一点时，哪怕击杀能获得 4000 经验的 Boss，也只能到三阶 8499；剩下的这一点经验强制要求打坐提升，以此类推。
""",
)

item(
    "jingjie", "realm_attributes", "属性提升", "境界,属性,数值",
    "每提升一个境界获得的生命、移速、灵力与攻击倍率加成。",
    """
凡境：生命 +0，移速 ×1，灵力 +0，攻击倍率 0

淬体：生命 +10，移速 ×1.05，灵力 +5，攻击倍率 +1.25

炼筋：生命 +15，移速 ×1.1，灵力 +10，攻击倍率 +1.5

辟谷：生命 +25，移速 ×1.15，灵力 +15，攻击倍率 +1.75

引气：生命 +40，移速 ×1.2，灵力 +20，攻击倍率 +2

入微：生命 +55，移速 ×1.25，灵力 +30，攻击倍率 +2.25

超凡：生命 +70，移速 ×1.3，灵力 +40，攻击倍率 +2.5

合婴：生命 +80，移速 ×1.35，灵力 +50，攻击倍率 +2.75

具灵：生命 +100，移速 ×1.4，灵力 +70，攻击倍率 +3
""",
)

item(
    "jingjie", "reiki_value", "灵力值", "境界,灵力,数值",
    "初始 50、上限 120；每分钟回复 3.3；枯竭时会大幅减速。",
    """
灵力值初始为 50，通过提升境界增加，上限 120。

每分钟回复 3.3 点灵力值。

灵力枯竭时（低于 5）移动速度降低 90%，持续 10 秒；虚弱解除后恢复 10 点灵力。
...
item(
    ...
    "开局的生物会随机入魔，入魔天数可调；Boss 必定深度入魔。",
    """
开局随机入魔，游玩天数可调节为 30 / 60 / 90 天，默认为 60 天。Boss 生物必定进入深度入魔。

无仇恨值时，入魔生物的生命值每秒回复 2%。

所有入魔生物均为敌对目标（切斯特这类中立生物除外；玩家的特定属性关系也除外，比如蜘蛛不会攻击蜘蛛人）。

入魔分三档：轻度入魔（魔气初染）、中度入魔（魔性觉醒）、深度入魔（魔核固化）。
""",
)

item(
    "rumo", "corruption_light", "轻度入魔（魔气初染）", "入魔,轻度",
    "生命值 +70%，会给玩家挂上魔气环绕、饥饿诅咒、暗影迟缓。",
    """
核心强化
生命值：+70%（例：原版 100 → 170，原版 2000 → 3400）

人物负面
魔气环绕：5 范围内玩家理智每 5 秒流失 1 点。
饥饿诅咒：玩家受到生物攻击时 30% 概率触发，饥饿值加快 20% 消耗，持续 30 秒。
...
item(
    "rumo", "corruption_medium", "中度入魔（魔性觉醒）", "入魔,中度",
    "生命值 +120%，带 15 点魔甲，半血触发无敌护盾，还有魔气反弹。",
    """
核心强化
生命值：+120%（例：原版 100 → 220，原版 2000 → 4400）
护甲：魔甲提升至 15 点（减免 12% 物理伤害）

人物负面
魔气环绕：10 范围内玩家理智每 5 秒流失 3 点。
饥饿诅咒：玩家受到生物攻击时 30% 概率触发，饥饿值加快 20% 消耗，持续 30 秒。
暗影迟缓：玩家受到生物攻击时 20% 概率触发，移速 -20%，持续 10 秒。
魔法反弹：生物受到玩家伤害时 10% 概率触发「魔气反弹」（反弹 2% 伤害）。

生物强化
限定易伤：生命值降低到 50% 会触发护盾，期间不受伤害（无敌状态），持续 25 秒，冷却 5 分钟。
吸食残骸：每当有玩家死亡，恢复自身损失生命值的 10%。
...
item(
    "rumo", "corruption_deep", "深度入魔（魔核固化）", "入魔,深度",
    "生命值 +180%，25 点魔甲，毒入骨髓、镜像混淆、魔化狂暴、祭天天劫。",
    """
核心强化
生命值：+180%（例：原版 100 → 280，原版 2000 → 5600）
护甲：魔甲提升至 25 点（减免 20% 物理伤害 + 5 位面防御）
移动速度 +10%，体型增大 1.2 倍。
...
暗影迟缓：玩家受到生物攻击时 20% 概率触发，移速 -20%，持续 10 秒。
魔法反弹：生物受到玩家伤害时 10% 概率触发「魔气反弹」（反弹 2% 伤害）。
镜像混淆：攻击时 10% 概率触发「镜像」，玩家接下来 5 秒内操作反向（左 = 右、前 = 后），期间无法攻击。

生物强化
限定易伤：生命值降低到 50% 会触发护盾，期间不受伤害（无敌状态），持续 25 秒，冷却 5 分钟。
毒入骨髓：普通攻击会附带毒伤（蚀骨毒）。单次中毒为 240 秒，不可叠加（可重置中毒时间，上限 240 秒）。
　0 到 80 秒：每 10 秒造成 6 点伤害
　80 到 160 秒：每 10 秒造成 10 点伤害
　160 到 240 秒：每 10 秒造成 14 点伤害
　时间结束则中毒效果结束。可使用特定丹药解毒，也可使用腺体直接解毒（会扣 50 血量；如果血量不足 50，则保留 5 点生命值）。
吸食残骸：每当有玩家死亡，恢复自身损失生命值的 10%。
影灵随行：唤醒时会额外召唤一个半血生命值的影体（影体不携带魔化效果，无护甲保护，魂幡可直接炼化）。
魔化狂暴：生命值低于 30% 时进入「魔化狂暴」——本体技能释放频率翻倍，伤害 +30%，魔甲脱落（无护甲保护），冷却 5 分钟（5 分钟内只会狂暴一次）。
祭天：死亡后召唤天劫，每 6 秒触发一次雷击，一击必杀，会有标识，出现标识 1.5 秒后落地，持续 60 秒。

击杀后掉落「魔晶」+ 生物原生材料（产量翻倍）。
""",
)
...
item(
    ...
    """
魔核碎片 lj_magic_debris：击杀轻度入魔生物掉落魔核碎片 1 到 10 不等。

魔核 lj_magic_core：击杀 Boss 生物、魔兽掉落；99 个魔核碎片合成；中度入魔生物掉落魔核 1 到 6 不等。无法摧毁、可分解。

魔晶 lj_magic_crystal：击杀魔兽掉落；20 个魔核合成；深度入魔生物掉落魔晶 1。无法摧毁、可分解。
...
丹炉炼制：""" + R(("鳞片", 1), ("玻璃碎片", 5), ("绿宝石", 1), ("魔晶", 1)) + """，炼制时间 4 分钟。
...
item(
    "danyao", "lj_reiki_gourd", "灵虚葫", "特殊炼制,法宝,储物",
    "吸收异火、存放丹药的 8 格葫芦；可跟随玩家并自动吸收 16 码内异火。",
    """
特殊炼制（不触发丹劫，炼制概率 100%）

丹炉炼制：""" + R(("魔核", 2), ("铥矿", 4), ("蓝宝石", 1), ("红宝石", 1)) + """，炼制时间 4 分钟。

不能摧毁，无法用分解法杖分解。

吸收异火、存放丹药，8 格空间。
右键打开 / 关闭。打开状态下会一直在，不会因打开其他箱子类物品而自动关闭葫芦。
可鼠标拿起放置地面跟随；跟随状态下 16 码范围内有异火，会自动吸收。
...
item(
    "danyao", "lj_soul_banner", "魂幡", "特殊炼制,法宝,影怪",
    "插入地面自动吸收影怪并转化为魔晶；给予紫鳞妖焰可解锁「炼魂」。",
    """
特殊炼制（不触发丹劫，炼制概率 100%）

炼制：""" + R(("噩梦燃料", 5), ("芦苇", 5), ("树枝", 5), ("魔晶", 2)) + """，炼制时间 4 分钟。

无法摧毁，可分解。

给予紫鳞妖焰可解锁技能「炼魂」。
插在地上可自动识别影怪类生物，吸入魂幡；每吸入一个单体影怪可转化成一个魔晶，魔晶会直接掉落在摆放位置的正下方。
""",
...
材料：""" + R(("融灵草", 3), ("蜂蜜", 3), ("桦栗果或松果", 3), ("魔核", 1)) + """。

效果：恢复 50 饱食度，猪人也爱吃。
""",
    recipe=R(("融灵草", 3), ("蜂蜜", 3), ("桦栗果或松果", 3), ("魔核", 1)),
)
...
item(
    ...
    "远古遗迹史诗 Boss 炽岩蝎龙的骨骼，用于制作骸龙甲与具灵丹。",
    """
来源：击杀远古遗迹史诗级 Boss「炽岩蝎龙」掉落 1 个。
...
item(
    "cailiao", "lj_snake_skin", "鳞片（噬魂蛇）", "材料,Boss机制",
...
# ===========================================================================
item(
    "jianzhu", "lj_alchemy_furnace", "丹炉", "建筑,炼制",
...
2 格空间。放入分裂出的墟火 / 玄焰可增加照明范围，一个加 10 码范围，维持 8 分钟；放入异火则永久提供照明。
...
item(
    "jianzhu", "lj_reiki_cultivatepool", "玄灵培育池", "建筑,灵植",
    "放入魂元妖树根供其生长的培育建筑。",
    """
...
item(
    ...
    """
特殊制作：靠近修复的远古残骸解锁，每次制作都需要靠近远古残骸。
...
item(
    ...
    "攻击 45、范围 1.5、手持加 10% 移速，可用魔核碎片修复的过渡兵刃。",
    """
制作：""" + R(("魔核碎片", 20), ("树枝", 5), ("绳子", 2), ("燧石", 2)) + """。

攻击 45，攻击范围 1.5，手持增加 10% 移动速度。

耐久 100（可攻击 100 下），可用魔核碎片填充，1 个填充 100。
...
攻击造成冰蚀伤害 +30 点（持续伤害，命中后增加「冰蚀」debuff），增加 10 点位面伤害。
攻击间隔 1.2 秒，射程 10，耐久 200。攻击一次减 1 点耐久。
...
技能：玄冰灌注（右键主动）
消耗 20 点灵力值，释放一只巨箭，造成 300 伤害，眩晕并冰冻目标 3 秒。冷却 12 秒。
...
焚灼伤害 +15 点（持续伤害，命中附加「焚灼」debuff，给生物挂一个燃烧状态，攻击挂 8 秒，每次攻击重置），增加 10 点位面伤害。
攻击间隔 1 秒，攻击距离 2，耐久 300。攻击一次减 1 点耐久。
耐久为 0 不消失，可继续攻击，但伤害降为 10。可用魔核填充耐久，1 个魔核填充 100。
...
技能：陨火刺（右键主动）
消耗 10 点灵力值，往前冲刺一段距离（借鉴女武神奔雷矛），过程中免疫伤害，类似无敌帧，造成 150 点伤害。冷却 2 秒。
...
# ===========================================================================
item(
    "fabao", "lj_magic_ring", "虚空戒", "法宝,工具,开局自带",
    "开局自带的戒指，一格空间可放墟火 / 异火，手持可点燃物品、焚烧建筑并使用灵技。",
    """
特殊制作：靠近修复的远古残骸解锁，每次制作都需要靠近远古残骸。

制作：""" + R(("魔核", 1), ("红宝石", 1), ("蓝宝石", 1), ("紫宝石", 1), ("黄宝石", 1), ("橙宝石", 1), ("紫宝石", 1)) + """。

不能摧毁，无法用分解法杖分解。

开局自带，一格空间，可放入墟火 / 异火（灵煊尘火放入可解锁灵技）。可手持使用灵技。

手持带有墟火 / 异火的虚空戒，可直接右键物品点燃、添加燃料、焚烧建筑，消耗 10 灵力值。

可在 <灵界>、<工具> 栏位制作。
""",
    recipe=R(("魔核", 1), ("红宝石", 1), ("蓝宝石", 1), ("紫宝石", 1), ("黄宝石", 1), ("橙宝石", 1), ("紫宝石", 1)),
)
...
范围：阵法范围根据阵旗相连划分，比如一个圆形、正 / 长方形，类似在地图上围一圈虚线，显示大概范围。阵旗最远 3 格范围相互连接。
...
item(
    ...
    "范围内农田作物春夏秋冬都正常生长。",
    """
范围：12×12，需要最少 4 阵旗（具体范围待实测后再修改）。
...
item(
    ...
    """
阵眼：灵植核心（""" + R(("鹿角", 1), ("绿宝石", 1), ("噩梦燃料", 5), ("金块", 5), ("魔核", 1)) + """）。
...
制作：""" + R(("鹿角", 1), ("绿宝石", 1), ("噩梦燃料", 5), ("金块", 5), ("魔核", 1)) + """。

用于生机回春阵：范围内提供光照，植物不枯萎、夜晚也能生长。

可在 <灵界>、<魔法> 栏位制作。
""",
    recipe=R(("鹿角", 1), ("绿宝石", 1), ("噩梦燃料", 5), ("金块", 5), ("魔核", 1)),
)
...
item(
    ...
    "多种丹药的基础药材。森林地皮蜘蛛巢与魔兽森林夜蝠巢穴附近生成。",
    """
生成：森林地皮的蜘蛛巢、魔兽森林夜蝠巢穴附近（每个巢穴附近 1 到 3 个随机）。
...
采摘：可获得 花瓣 1、彼岸花 1；5% 概率额外获得 彼岸花种 1。
...
虚空戒点燃：手持带有墟火 / 异火的虚空戒，可直接右键物品点燃、添加燃料、焚烧建筑，消耗 10 灵力值。
...
item(
    ...
    """
异火。根据线索寻找到的 Boss 生物均为深度入魔，无视季节；线索物品 20 天一刷新。

线索物品：「冻僵的日志」lj_frozen_log（桦树林随机刷新，需要用镐子敲碎解锁）。
...
线索物品：「焦黑鳞片」lj_charred_scales（岩浆池旁边刷新，需要用水壶浇灭）。
...
线索物品：「腐烂的背包」lj_rotten_backpack（森林地形随机刷新，需要用斧头劈开解锁）。

获取流程：获取日志后右键打开，纸条记载着「它推倒松树时，树根下会冒出火星，怕水却爱蜂蜜」。地图上会随机生成一个蜂箱，过去会看到熊獾在蜂箱旁边，击杀会掉落正常掉落物，并额外掉落狂鬃焰。
...
item(
    ...
    "通过「风干的羽毛」线索召唤蚁狮获得的异火，放入虚空戒可解锁灵技。",
    """
异火。

线索物品：「风干的羽毛」lj_dried_camel_feathers（绿洲附近拾取）。
也可使用丹炉炼制：""" + R(("仙人掌", 3), ("风干的骆驼羽毛", 1)) + """。
...
线索物品：「泡胀的卷轴」lj_swollen scroll（沼泽触手旁随机刷新，需要火焰烘烤解锁）。

获取流程：获取日志后右键打开，纸条记载着「薄暮将至，灵脉水面会倒映出紫色火光」。查看日志后，Boss 会在黄昏时分出现在魔兽森林的池塘旁边。过去会看到噬魂三头蛇，击杀会掉落正常掉落物，并额外掉落紫鳞妖焰。
...
item(
    ...
    "异火线索所用的日志。线索 Boss 均为深度入魔，线索物品 20 天一刷新。",
    """
异火线索物品的统称。

根据线索寻找到的 Boss 生物均为深度入魔，无视季节。

线索物品 20 天一刷新。线索 Boss 均为深度入魔。
...
右键打开后纸条记载着「它推倒松树时，树根下会冒出火星，怕水却爱蜂蜜」。地图上会随机生成一个蜂箱，过去会看到熊獾在蜂箱旁边。
""",
)
...
右键打开后纸条记载着「薄暮将至，灵脉水面会倒映出紫色火光」。查看日志后，Boss 会在黄昏时分出现在魔兽森林的池塘旁边，过去会看到噬魂三头蛇。
""",
)
...
# ===========================================================================
item(
    "ditu", "relic_ruins", "远古遗迹", "地形,特殊制作",
    "浅海区的小地形，生成远古残骸与炽岩蝎龙；修复残骸可解锁特殊制作。",
    """
小地形，生成远古残骸，生成炽岩蝎龙，生成在浅海区。

建造修复残骸：""" + R(("木板", 10), ("融灵草", 3), ("紫晶塑体花", 1), ("化石碎片", 3)) + """。
须至少辟谷境才可以修复；修复完成掉落一个大境界（一共掉落九阶）。

靠近解锁制作（每次制作都需要靠近远古残骸，不会因只做过一次而解锁配方）：
灵韵、星陨、1 到 3 级魂幡、骸龙甲、晶羽冠、四季核心、灵植核心、镇魔枢核、恒温晶核、玄阵枢纽、虚空戒。
""",
)

item(
    "ditu", "lj_chiyan_scorpion_dragon", "炽岩蝎龙", "Boss,史诗,远古遗迹",
    "远古遗迹史诗级 Boss。血量 30000、护甲 30；不吃冰冻僵直击退，半场后召唤毒蝎幼虫。",
    """
生成在远古遗迹。史诗级 Boss。
...
技能
横扫（普攻）：钳子挥击，范围 4 格，每钳 60 伤，冷却 4 秒。攻击 4 下后尾巴横扫造成 90 点 AOE 范围伤害，并对玩家造成 2 格击退效果。

陨石崩击：召唤一个从天而降的陨石，飞速砸向玩家，类似陨石区的石头坠落（破坏建筑及树木，同原版陨石区）。冷却 15 秒。造成 120 点伤害，并击倒玩家（让玩家击飞摔倒的样子）。

沙砾喷发：身前 5 格锥形喷沙，减速 40% + 每秒 10 伤，持续 3 秒，冷却 15 秒。
　喷发持续释放时长 3 秒（全程持续判定伤害与减速），身前 5 格锥形区域，宽 3 格，持续生成沙砾腐蚀地形，喷发结束后地形留存 2 秒。
　减速 DEBUFF：沙砾内移速降低 40%，离开区域后减速效果立刻清除。

蝎龙冲撞：蓄力 2 秒直线冲锋 8 格，100 伤害 + 摧毁路径建筑，结束后眩晕 2 秒，冷却 30 秒。直线最大距离 8 格。
　建筑破坏：路径内所有木、石制墙体、采集物、小型建筑直接摧毁。全局冷却 30 秒。
...
item(
    "ditu", "lj_little_scorpion", "毒蝎幼虫", "Boss,召唤物,远古遗迹",
...
item(
    "ditu", "lj_warcraft_forest", "魔兽森林", "地形,季节,魔兽",
    "分为春夏秋冬四个板块的地域。首版开放冬季板块，是异火线索 Boss 与魔兽的聚集地。",
    """
进入魔兽森林后，分为四个板块，对应四个季节（春夏秋冬）。首版冬季板块为全天数冬天，剩下三个板块一样会有季节效应，内容后续更新。（地域结界：上岛才可以感受地域季节）

冬季地域板块
核心区域（沼泽地皮）：有一个冰冻起来的沼泽池塘（线索 Boss 刷新处）；两棵完全正常的树，围着一些邪恶花；三棵阴郁之棘；若干针刺树，若干尖刺灌木。
...
item(
    "ditu", "lj_bat_nest", "夜蝠巢穴", "地形,魔兽森林,刷怪",
    "每个巢穴生成 6 只妖蝠与 1 只暗影血蝠，黄昏夜晚出没。",
    """
魔兽森林生成若干。每个巢穴生成 6 只妖蝠（5 分钟刷新 1 只，最多 6 只）、1 只暗影血蝠（45 分钟刷新 1 只，最多 1 只）。
...
item(
    "ditu", "lj_blood_bat", "妖蝠", "魔兽,魔兽森林,蝙蝠",
    "小魔兽。血量 300、伤害 25，固定掉落怪物肉或蝙蝠翅膀。",
    """
初始血量 300，攻击伤害 25，攻击频率 5 秒。

攻击方式：1 格地皮内撕咬，超范围冲刺攻击。冲刺击中会有短暂的僵直效果，可提前上下走动躲避。

击杀固定掉落 怪物肉 1 或者 蝙蝠翅膀 1（魔核碎片、魔核、魔晶根据当前入魔程度掉落）。非大型魔兽生物。
...
item(
    "ditu", "lj_demon_bat", "暗影血蝠", "魔兽,魔兽森林,蝙蝠,精英",
...
音波攻击：释放暗红色音波，以自身为中心 4×4 地皮范围攻击，范围内持续僵直，每 1.5 秒僵直 0.5 秒，持续 9 秒。定身结束走出范围即可。冷却 30 秒。
...
item(
    "ditu", "lj_soul_devouring_snake", "噬魂蛇（隐藏 Boss）", "Boss,史诗,隐藏,魔兽森林",
    "血量 12000 的隐藏 Boss。每受 2000 伤掉鳞片，半血后会吞鳞回血 1000。",
    """
隐藏 Boss，史诗级。

基础属性
血量 12000，普攻撕咬 40 伤害，附加中毒效果，护甲 15%，移速中等。

核心机制
每受 2000 血量伤害会掉落鳞片 lj_snake_skin，玩家无法拾取，可用异火烧毁。
...
定身石化：以自身为中心 4×4 范围内，定身目标 3 秒，冷却 30 秒。
...
基础属性
血量 24000，普攻 60，攻击间隔 2.8 秒，护甲 15%，移速中等。
...
技能
晶爪猛击：单体近战重击。伤害 70，普攻冷却 4 秒，范围身前 3 格矩形，击倒小型生物，玩家受击硬直 0.6 秒。如果玩家距离小于 1 格，则 60% 概率左右手各触发一次，40% 概率正常攻击。
鳞粉飞弹（远程消耗）：扇动单侧翅膀发射 3 枚蝶粉光球，呈三角散射。单发伤害 40，三发全中 120。单颗爆炸半径 1.5 格，爆炸后会在爆炸点生成一个花瓣。冷却 12 秒，飞行距离 8 格，遇障碍物直接爆炸。
狮吼震慑（控制技能）：全屏 6 格范围声波冲击。无伤害，但玩家僵直 1.2 秒，武器强制掉落地面。冷却 10 秒。
裂地囚步：狮子抬起单侧前爪，身躯下沉蓄力，地面轻微持续震动，脚下扬起细碎沙尘特效，屏幕小幅抖动提示玩家规避。
　生成范围：以自身 8 个地皮为中心，对周围的玩家临时生成限时陷坑，完全不摧毁建筑、地皮、墙体、作物，2 秒自动消散（区别于蚁狮的永久塌陷坑）。
　单个玩家生成一个小坑，多名玩家则几名玩家就几个坑。
　坑体存在 3 秒，所有踏入范围内的敌方单位持续承受伤害与减速 debuff；3 秒倒计时结束立刻清空坑体、清除全部负面效果。
　两段伤害每段 60，坑内减速 70%。
...
DATA = {
    ...
    "items": ITEMS,
    "changelog": [
        {"日志版本": "v0.1.0", "日期": "2026-06-23", "内容": "导入灵界设定资料。", "是否展示": "true"},
    ],
