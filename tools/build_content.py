...
从炼筋突破到辟谷开始，之后每一次跨大境界（辟谷、引气、入微、超凡、合婴、具灵）都须服用对应丹药并打坐 10 秒才能提升境界。
（设计文档写「引气及以上」，实际门槛从辟谷就开始了；共 6 种突破丹药。）
""",
)
...
⚠️ 设计文档写的「移速降低 90%」在 mod 代码里没有对应的移速数值，实际效果是上述原版睡眠/昏沉机制。
...
注：mod 代码里没有"魔甲 15 点"这种点数体系，设计文档的"15 点"只是对 12% 减伤的另一种说法。
...
　　注：设计文档说"期间无法攻击"，但 mod 代码里找不到与之关联的攻击封锁逻辑，实际只反转移动方向。
...
（设计文档写「1 个填充 100」是错的：回复 100 是魔核的数值，不是魔核碎片。）
...
攻击命中后附加「冰蚀」减益：持续 8 秒，每 0.5 秒造成 16 点伤害，再次命中会重置时间。增加 10 点位面伤害（这个数值与文档一致）。
...
攻击命中后附加「焚灼」减益：持续 8 秒，每 0.5 秒造成 16 点火焰伤害，再次命中会重置时间。增加 10 点位面伤害（这个数值与文档一致）。
...
　⚠️ 设计文档写的「减速 40% + 每秒 10 伤」在 mod 代码里没有实现：减速与伤害都来自原版沙刺（`sandspike`）本身，不是本技能附加的。
...
item(
    ...
    """
⚠️ 命名说明：设定文档里把这里叫做「魔兽森林」，但 mod 源码里实现为蝴蝶岛（`lj_butterfly_island`），
...
音波攻击：释放暗红色音波，以自身为中心 半径 8 的范围攻击，每 0.5 秒一次脉冲，范围内持续僵直，持续 9 秒。走出范围即可解除。冷却 30 秒。（设计文档写「每 1.5 秒僵直 0.5 秒 / 4×4 地皮」，实际是每 0.5 秒一跳、半径 8）
...
定身石化：以自身为中心、半径 16 的圆形范围内，定身目标 3 秒，冷却 30 秒。（文档写「4×4 范围」，实际是圆形）
...
基础属性
血量 24000，护甲 15%，移速中等。
⚠️ 本体没有设置普攻伤害，也没有攻击间隔——设计文档写的「普攻 60、攻击间隔 2.8 秒」在代码里不成立，伤害全部来自下面几个技能。
...
　两段伤害每段 60。
　⚠️ 设计文档写的「坑内减速 70%」在代码里没有实现；代码另有每秒 1 点的持续伤害。
