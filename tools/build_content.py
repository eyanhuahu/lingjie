...
从**炼筋突破到辟谷**开始，之后每一次跨大境界（辟谷、引气、入微、超凡、合婴、具灵）都须服用对应丹药并打坐 10 秒才能提升境界。
（设计文档写「引气及以上」，实际门槛从**辟谷**就开始了；共 6 种突破丹药。）
""",
)
...
灵力枯竭时（低于 5）会进入 10 秒的虚弱状态：按原版规则叠加睡意与昏沉，甚至直接昏睡（knockout）；虚弱解除后恢复 10 点灵力。
⚠️ 设计文档写的「移速降低 90%」在 mod 代码里**没有对应的移速数值**，实际效果是上述原版睡眠/昏沉机制。
...
item(
    ...
    """
生物生成时随机抽入魔等级。游玩天数可调节为 30 / 60 / 90 天，默认为 60 天——只有到达该天数后才可能抽到深度入魔。Boss（`epic`）生物不受天数限制，必定深度入魔。

回血：脱离有效目标连续 10 秒后，每 5 秒回复最大生命的 2%（受到攻击会重置这个脱战计时）。不是"每秒回 2%"。
...
item(
    ...
    "生命加成随基础血量递减；会给玩家挂上魔气环绕、饥饿诅咒、暗影迟缓。",
    """
（设计文档里叫「魔气初染」，游戏内只显示「轻度」。）

核心强化 —— 生命值
加成**不是固定百分比**，而是随基础血量递减的公式：

　　最大生命 = 基础生命 + (基础生命 ÷ (基础生命 + 1500)) × 70% × 1500
...
额外生命上限 = 70% × 1500 = 1050，所以基础血量越高，涨幅百分比越低。衰减阈值可在 mod 设置里改成 1000 / 1500 / 3000。
...
item(
    ...
    "生命加成随基础血量递减；带 12% 减伤、半血触发无敌护盾、5% 魔气反弹。",
    """
（设计文档里叫「魔性觉醒」，游戏内只显示「中度」。）

核心强化 —— 生命值
　　最大生命 = 基础生命 + (基础生命 ÷ (基础生命 + 1500)) × 120% × 1500
...
额外生命上限 = 120% × 1500 = 1800。

核心强化 —— 护甲
受到的所有外部伤害 ×0.88（即减免 12% 物理伤害）。
注：mod 代码里**没有"魔甲 15 点"这种点数体系**，设计文档的"15 点"只是对 12% 减伤的另一种说法。
...
item(
    ...
    "生命加成随基础血量递减；免疫冰冻睡眠、无视减速、毒入骨髓、镜像混淆、魔化狂暴、祭天天劫。",
    """
（设计文档里叫「魔核固化」，游戏内只显示「深度」。）

核心强化 —— 生命值
　　最大生命 = 基础生命 + (基础生命 ÷ (基础生命 + 1500)) × 180% × 1500
...
　　注：设计文档说"期间无法攻击"，但 mod 代码里**找不到与之关联的攻击封锁逻辑**，实际只反转移动方向。

生物强化
限定易伤：生命值在 30% 与 50% 之间（>30% 且 ≤50%）时触发无敌护盾 25 秒，冷却 5 分钟。
　　注：中度是无条件「≤50% 触发」，深度多了「必须 >30%」这个下界。
...
魔化狂暴：生命值低于 30% 时进入狂暴——伤害 +30%、魔甲脱落（受伤乘数与位面防御加成一并移除）、技能释放频率翻倍（计时减半），冷却 5 分钟。
　　狂暴期间**完全无法回血**（任何治疗都被拦下）。
...
给予紫鳞妖焰可解锁技能「炼魂」。
插在地上可自动吸收**固定的 4 种影怪**（恐怖喙、爬行梦魇、梦魇喙、爬行梦魇）以及深度入魔产生的影体分身，吸入魂幡；每吸入一个单体影怪可转化成一个魔晶，魔晶会直接掉落在摆放位置的正下方。
（设计文档笼统写「影怪类生物」，实际是写死的 4 个预制体 + 影体分身。）
""",
...
item(
    ...
    """
来源：击杀蝴蝶岛（设定文档称「魔兽森林」）中的暗影血蝠固定掉落 1 个。
...
耐久 100（可攻击 100 下），可用**魔核碎片**修复，1 个魔核碎片回复 **10** 点耐久。
（设计文档写「1 个填充 100」是错的：回复 100 是**魔核**的数值，不是魔核碎片。）
...
攻击命中后附加「冰蚀」减益：持续 **8 秒**，每 **0.5 秒**造成 **16** 点伤害，再次命中会重置时间。增加 10 点位面伤害（这个数值与文档一致）。
...
技能：玄冰灌注（右键主动）
消耗 20 点灵力值，释放一只巨箭，造成 300 伤害，并**冻结**目标 3 秒。冷却 12 秒。
（设计文档写「眩晕并冰冻」，mod 代码里只有冻结，没有眩晕。）
...
攻击命中后附加「焚灼」减益：持续 **8 秒**，每 **0.5 秒**造成 **16** 点火焰伤害，再次命中会重置时间。增加 10 点位面伤害（这个数值与文档一致）。
（设计文档写「焚灼伤害 +15 点」，实际不是平砍加 15，而是上面这套持续伤害。）
...
开局自带，一格空间，可放入墟火 / 异火。手持带火的虚空戒右键可点燃物品、添加燃料（各消耗 10 点灵力）。
⚠️ 设计文档提到「手持使用灵技」「灵煊尘火放入可解锁灵技」，但 **mod 源码里查不到灵技系统的任何实现**（全库没有相关预制体与字符串），暂按未实装处理。
...
范围（mod 代码实际参数，1 地皮 = 4 游戏单位）
　最少需要 **8 面阵旗**才能成阵。
　阵旗之间**最少相隔 3 地皮**（12 单位），**最远相连 6 地皮**（24 单位）。
　整个阵法的外接矩形长、宽均**不得超过 12 地皮**（48 单位）。
　阵法范围根据阵旗相连划分，比如一个圆形、正 / 长方形，类似在地图上围一圈虚线，显示大概范围。

启动：阵眼需放入玄阵枢纽启动。玄阵枢纽有 4 个格子，可放入多个阵眼提供多重效果。
多个相同阵眼**不会叠加数值**，只是把不同的效果位并集起来。
...
item(
    ...
    "范围内农田作物春夏秋冬都正常生长。",
    """
范围：最少 8 面阵旗；阵法外接矩形不超过 12×12 地皮。
注：设计文档写「需要最少 4 阵旗」「12×12 待实测」——mod 代码里最少阵旗数固定为 **8**，
而 12×12 是**所有阵法**共同的外接矩形上限，不是本阵专属。
...
item(
    ...
    """
生成（两批，地皮条件不同）
　主大陆：长在森林地皮（`WORLD_TILES.FOREST`）的蜘蛛巢旁。
　蝴蝶岛：长在鸟粪 / 洞穴地皮（GUANO）上的夜蝠巢穴旁，每个巢穴 1 到 3 株。

采摘：可获得 草 1、融灵草 1；5% 概率额外获得 融灵草根 1。

生长周期同原版树枝，可催熟。
⚠️ **冬季不生长**——mod 给融灵草挂了 `MakeNoGrowInWinter`，设计文档写的「全季节生长」是不对的。
...
用途：放入虚空戒。
⚠️ 设计文档说它「放入虚空戒可解锁灵技」，但 mod 源码里查不到灵技系统的实现，暂按未实装处理。
""",
)
...
获取流程：获取日志后右键打开，纸条记载着「薄暮将至，灵脉水面会倒映出紫色火光」。查看日志后，Boss 会在黄昏时分出现在蝴蝶岛（设定文档称「魔兽森林」）的池塘旁边。过去会看到噬魂三头蛇，击杀会掉落正常掉落物，并额外掉落紫鳞妖焰。
...
它还会被用作**炼丹炉**的配方材料，用于炼制「尘火日志」（见该条目）。
""",
)
...
右键打开后纸条记载着「薄暮将至，灵脉水面会倒映出紫色火光」。查看日志后，Boss 会在黄昏时分出现在蝴蝶岛（设定文档称「魔兽森林」）的池塘旁边，过去会看到噬魂三头蛇。
""",
)
...
阅读需要境界达到 **入微**（`required_realm_major = 5`）。
""",
)
...
须至少辟谷境才可以修复；修复完成**会下降一个大境界（损失九阶）**——这是修复的代价，不是奖励。境界会保留原有的死亡锁定状态。

靠近解锁制作（`nounlock`，每次制作都需要靠近残骸祭坛，不会因只做过一次而永久解锁配方）：
灵韵、星陨、魂幡、骸龙甲、晶羽冠、四季核心、灵植核心、镇魔枢核、恒温晶核、玄阵枢纽、虚空戒。
（设计文档写的「1 到 3 级魂幡」不存在，魂幡在 mod 里没有等级。）
...
技能（括号内为 mod 代码实际数值；与设计文档不同处已注明）
横扫（普攻）：钳子挥击，每钳 60 伤，冷却 **8 秒**（文档写 4 秒）。攻击范围约 **1 格**（代码是 4 码，1 格 = 4 码；文档写「4 格」偏大了 4 倍）。攻击 4 下后尾巴横扫造成 90 点 AOE 范围伤害，并对玩家造成 2 格击退效果。

陨石崩击：召唤一个从天而降的陨石，飞速砸向玩家，类似陨石区的石头坠落（破坏建筑及树木，同原版陨石区）。冷却 **18 秒**（文档写 15 秒）。造成 120 点伤害，并击倒玩家（让玩家击飞摔倒的样子）。

沙砾喷发：身前 5 格锥形喷沙，持续 3 秒，冷却 **20 秒**（文档写 15 秒）。
　喷发持续释放时长 3 秒，身前 5 格锥形区域，宽 3 格，持续生成沙砾腐蚀地形，喷发结束后地形留存 2 秒。
　⚠️ 设计文档写的「减速 40% + 每秒 10 伤」在 mod 代码里**没有实现**：减速与伤害都来自原版沙刺（`sandspike`）本身，不是本技能附加的。

蝎龙冲撞：蓄力 **1 秒**（文档写 2 秒）直线冲锋 8 格，100 伤害 + 摧毁路径建筑，结束后眩晕 2 秒，冷却 30 秒。直线最大距离 8 格。
...
item(
    ...
    """
⚠️ 命名说明：设定文档里把这里叫做「魔兽森林」，但 mod 源码里实现为**蝴蝶岛**（`lj_butterfly_island`），
全代码搜索不存在 `lj_warcraft_forest` 这个预制体，中文语言文件里也没有「魔兽」「森林」任何字样。
本条目按代码叫法命名，并把「魔兽森林」放进标签，搜索旧名同样能找到。
...
item(
    ...
    """
初始血量 300，攻击伤害 25，攻击周期 **1 秒**（沿用原版 `BAT_ATTACK_PERIOD`；设计文档写「5 秒」是错的）。

攻击方式：1 格地皮内撕咬，超范围冲刺攻击。冲刺击中会有短暂的僵直效果，可提前上下走动躲避。

击杀**只固定掉落 怪物肉 1**（魔核碎片、魔核、魔晶根据当前入魔程度掉落）。非大型魔兽生物。
⚠️ 设计文档写的「或者 蝙蝠翅膀 1」不成立——mod 全仓库没有 `batwing` 这个掉落。
...
音波攻击：释放暗红色音波，以自身为中心 **半径 8** 的范围攻击，**每 0.5 秒**一次脉冲，范围内持续僵直，持续 9 秒。走出范围即可解除。冷却 30 秒。（设计文档写「每 1.5 秒僵直 0.5 秒 / 4×4 地皮」，实际是每 0.5 秒一跳、半径 8）
...
基础属性
血量 12000，普攻撕咬 **50** 伤害（设计文档同时写了 40 和 50，40 是错的），附加中毒效果，护甲 15%，移速中等。
...
定身石化：以自身为中心、**半径 16 的圆形范围**内，定身目标 3 秒，冷却 30 秒。（文档写「4×4 范围」，实际是圆形）
...
基础属性
血量 24000，护甲 15%，移速中等。
⚠️ 本体**没有设置普攻伤害，也没有攻击间隔**——设计文档写的「普攻 60、攻击间隔 2.8 秒」在代码里不成立，伤害全部来自下面几个技能。
...
技能
晶爪猛击：单体近战重击。伤害 70，冷却 **8 秒**（文档写 4 秒），范围约 **1 格**（代码是 3 码；1 格 = 4 码），击倒小型生物，玩家受击硬直 0.6 秒。如果玩家距离小于 1 格，则 60% 概率左右手各触发一次，40% 概率正常攻击。
鳞粉飞弹（远程消耗）：扇动单侧翅膀发射 3 枚蝶粉光球，呈三角散射。单发伤害 40，三发全中 120。爆炸半径约 0.4 格（代码是 1.5 码，文档写「1.5 格」偏大），爆炸后会在爆炸点生成一个花瓣。冷却 **18 秒**（文档写 12 秒），飞行距离 8 格，遇障碍物直接爆炸。
狮吼震慑（控制技能）：全屏 6 格范围声波冲击。无伤害，但玩家僵直 1.2 秒，武器强制掉落地面。冷却 **20 秒**（文档写 10 秒）。
...
　两段伤害每段 60。
　⚠️ 设计文档写的「坑内减速 70%」在代码里**没有实现**；代码另有每秒 1 点的持续伤害。
...
DATA = {
    ...
    "items": ITEMS,
    "changelog": [
        {"日志版本": "v0.1", "日期": "2026-09-15", "内容": "对照 mod 源代码全面校对：物品 id 与官方中文名改用代码实际值、配方与材料名修正、入魔与各 Boss 数值修正、标注未实装内容。", "是否展示": "true"},
