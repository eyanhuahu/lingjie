...
⚠️ 灵力枯竭不会降低移速，实际效果就是上述原版睡眠/昏沉机制。
...
⚠️ 「手持使用灵技」「灵煊尘火放入可解锁灵技」目前都不会生效：mod 源码里查不到灵技系统的任何实现（全库没有相关预制体与字符串）。
...
生长周期同原版树枝，可催熟。
⚠️ 冬季不生长——mod 给融灵草挂了 `MakeNoGrowInWinter`。
...
　⚠️ 减速与伤害都来自原版沙刺（`sandspike`）本身，不是本技能附加的。
...
item(
    ...
    """
⚠️ 命名说明：mod 源码里的预制体就叫蝴蝶岛（`lj_butterfly_island`），全代码不存在 `lj_warcraft_forest`。
标签里也放了「魔兽森林」，用这个别名搜索同样能找到。
...
item(
    ...
    """
初始血量 300，攻击伤害 25，攻击周期 1 秒（沿用原版 `BAT_ATTACK_PERIOD`）。
...
基础属性
血量 24000，护甲 15%，移速中等。
⚠️ 本体没有设置普攻伤害，也没有攻击间隔，伤害全部来自下面几个技能。
...
技能
晶爪猛击：单体近战重击。伤害 70，冷却 8 秒，范围约 1 格（3 码），击倒小型生物，玩家受击硬直 0.6 秒。如果玩家距离小于 1 格，则 60% 概率左右手各触发一次，40% 概率正常攻击。
...
DATA = {
    ...
    "changelog": [
        ...
        {"日志版本": "v0.1.0", "日期": "2026-06-23", "内容": "站点上线，导入首批词条。", "是否展示": "true"},
    ],
